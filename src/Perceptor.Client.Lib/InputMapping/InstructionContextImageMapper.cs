...
namespace Perceptor.Client.Lib.InputMapping
{
	internal static class InstructionContextImageMapper
	{
		...
		internal static async Task<IReadOnlyList<InstructionContextData>> MapFromStreams(IEnumerable<(Stream Str, string FileType)> imageStreams)
		{
		...
		internal static IReadOnlyList<InstructionContextData> MapFromBytes(IEnumerable<(byte[] imageBytes, string fileType)> imageInfos) =>
